...
В шапке есть форма «ID игры или ссылка → Собрать»: она ставит задачу в очередь (Celery + Redis) и опрашивает её статус, не блокируя графики. Задача проверяет игру в Steam, качает appdetails, новости и отзывы, грузит их в core и в конце пересобирает витрины. Глубину отзывов задаёт `COLLECT_REVIEW_DAYS` (365): сбор идёт от свежих к старым и останавливается, когда страница ушла за границу. `COLLECT_REVIEW_PAGES` (200 страниц по 100 отзывов) - страховка от бесконечного обхода у очень шумной игры, останавливает то условие, что сработало раньше. Новости берёт `COLLECT_NEWS_PAGES` (10). Отдельно от сбора: `DEFAULT_APP_ID` задаёт игру, на которой дашборд открывается; при пустом значении берётся первая по алфавиту.

Год достаётся не всем. У популярной игры 200 страниц - это 20 тысяч отзывов, и уходят они за недели, а не за год: полный сбор Rust упёрся в предел страниц и принёс 55 дней истории за 9 минут, тогда как у тихой Medieval Dynasty 58 страниц покрыли 366 дней за 3.5 минуты. Из 21 игры в предел упираются 19, а у восьми самых шумных 200 страниц не дотягивают даже до 90 дней, нужных для базы. За глубиной идти коллектором руками: `collector/fetch_reviews.py` принимает предел страниц числом и `--days` той же границей по дате.
