...
Если в окружении задан `COLLECT_TOKEN`, `/api/collect` отвечает
только на запросы с заголовком `X-Collect-Token`, равным ему,
остальным — 401; без переменной форма работает как раньше. Токен
уезжает в html страницы, так что это защита от случайного запроса,
а не от целенаправленного — настоящая будет basic auth в nginx
при деплое.
