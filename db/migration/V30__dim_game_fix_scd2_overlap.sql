-- Точечная правка в psql (запись 006) переписала valid_from на 2000-01-01 не только у первой версии игры, но и у более поздних,
-- и интервалы [valid_from, valid_to) внутри одного app_id стали пересекаться. 
-- Файл восстанавливает границы, перепривязывает факты по фактическому моменту и запрещает пересечения на будущее.
