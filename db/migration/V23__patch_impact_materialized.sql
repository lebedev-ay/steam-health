-- patch_impact из представления становится материализованным: cross
-- join событий с окнами и вложенный цикл по датам пересчитывались
-- на каждый клик в дашборде (decisions.md, запись 018). Определение
-- взято из V20 без изменений, меняется только вид объекта. review_flat
-- остаётся представлением - после V21 он не разворачивает JSON,
-- и пересчёт у него дешёвый.
