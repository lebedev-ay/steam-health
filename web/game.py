...
def words(app_id, since, until, vote):
    # в ключе кэша - час: ночной сбор дописывает отзывы за последние дни, и тот же период к утру должен пересчитаться
    return _words(app_id, since, until, vote, int(time.time() // 3600))


@lru_cache(maxsize=512)
def _words(app_id, since, until, vote, _hour):
    ...
    Кэш на процесс и на час: запрос недорогой, но его зовут на каждую открытую карточку перелома.
    """
    rows = query("""
        with docs as (
