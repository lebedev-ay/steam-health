...
-- границы собранных данных: за ними окно неполное и сравнивать
-- его с полным нельзя (см. decisions.md, запись 009)
with bounds as (
    select
        app_id,
...
-- пара «событие + окно» разворачивается в календарные даты, чтобы
-- складывать дневные суммы по равенству - decisions.md, запись 030
expanded as (
    select
        p.patch_sk,
        p.app_id,
